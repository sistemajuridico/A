...
def extrair_json_seguro(texto_bruto):
    ...
    try:
        ...
        raise Exception("Erro na formatação da análise. Tente novamente.")
...
def processar_background(task_id: str, fatos: str, area: str, mag: str, trib: str, caminhos: list):
    try:
        ...
        sys_instr = f"""Você é o ANALISTA ESTRATÉGICO do M.A JUS IA EXPERIENCE. Especialidade: {area}.
        Sua missão é realizar uma AUDITORIA JURÍDICA COMPLETA no processo. 
        Não redija petições. Foque em encontrar contradições, nulidades e brechas.
        REGRAS JSON: Use apenas aspas simples nos textos internos."""
        
        prompt = f"DIRECIONAMENTO:\n{fatos}\n\nJuízo: {mag} | Vara: {trib}"
        ...
        # CORRIGIDO: Agora usa TASKS corretamente
...
def gerar_docx(dados: DadosDossie):
    ...
    doc.save(buf)
    buf.seek(0)
    return StreamingResponse(buf, media_type="application/vnd.openxmlformats-officedocument.wordprocessingml.document", headers={"Content-Disposition": "attachment; filename=MA_Analise.docx"})
